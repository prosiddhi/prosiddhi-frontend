...
| **Sign-in (R1-BE-01, 2026-09-21)** | `REQUIRE_PHONE_VERIFICATION=false` · `ADMIN_CONSOLE_ORIGINS` · `OTP_IP_RATE_LIMIT_MAX` | ❌ | **`REQUIRE_PHONE_VERIFICATION=false`** (exactly `true`/`false`, lowercase) for seekers and employers until SMS delivers: registration skips the phone code (`phoneVerified: false`); phone-OTP login and phone password reset answer `503` (`PHONE_OTP_LOGIN_DISABLED` / `PHONE_PASSWORD_RESET_DISABLED`); password login and email verification unchanged. Check after restart: the boot log lists it under "DANGER FLAGS SET", and `GET /api/v1/auth/config` returns `data.requirePhoneVerification: false`. Reverse in January: remove the line (default `true`) and restart. **`ADMIN_CONSOLE_ORIGINS`** — default `https://admin.prosiddhi.com`; set only if the console is served from another origin, or staff password reset stops working. **`OTP_IP_RATE_LIMIT_MAX`** — default 20 code sends per IP per 15 min; leave unset. Turning `EXPOSE_OTP_IN_RESPONSE` off is **not** this — that is R1-OPS-06, after email delivery is proven |
