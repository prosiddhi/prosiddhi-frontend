...
| 33 | Portal | **NEW — open, low.** Date of birth is now editable on the seeker profile (`5c9c947`) with no 18+ check in the browser (`profile/page.tsx:626-633`). The backend still refuses under-18 (`auth.validator.ts:29-43`), so no rule is broken — but the user sees the raw backend message, untranslated. Registration's `isAtLeast18` could be reused |
