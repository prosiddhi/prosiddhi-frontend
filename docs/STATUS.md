...
**The single source of truth for what is done and what is left.** Updated **2026-09-15** — the portal from its own diffs, and the backend, admin and mobile from their sync reports (`C:\dev\Azkashine\Prosiddhi\_status-sync\`).
...
**Latest (2026-09-15): the portal was redesigned, the backend changed underneath all three clients, and production caught up. 19 new portal bugs — 14 from the redesign, 5 from backend contract changes.**

**What happened.** Between 2026-08-25 and 09-11, **Bharath Kumar** made **40 portal commits** (172 files, +10,421 / −4,394 lines). Most screens now follow the Figma file. **Nothing was lost:** every flow the portal had on 08-21 still works, and every API call still hits a real backend route — each one was checked against `prosiddhi-backend/src` on 2026-09-15. What is new:

- **A seeker home page.** After login a seeker lands on `/home`: a search box plus the All / Recommended / Near By tabs. The job feed became a search page with a filter sidebar.
- **Seekers and employers now have separate pages** for job details, messages and settings. Example: "View" on My Jobs opens `/employer/jobs/<id>`, and an employer who opens an old `/job-details/<id>` link is sent there automatically. No old link breaks.
- **Profiles can change email and phone**, confirmed by a one-time code — the gap DEF-032 described. Both profiles now open read-only with an Edit button.
- **Login rebuilt again** (React Hook Form + Zod). Still one form for phone-or-email + password, a code-by-SMS option, and Google.
- **Forgot password takes a phone number** as well as an email.
- **The password rule now needs a special character**, with a live checklist.
- **Employers got a credit ledger** at `/employer/ledger` — every credit earned or spent, with filters. Owner only.
- **Job moderation notifications** — an employer is told when an admin approves or rejects their job.

**Production caught up.** TD-00 (deploy `main`) is done in effect. On 2026-09-15 production served `/home` and `/employer/ledger`, and a made-up URL returned 404, so the probe is real. It does **not** yet have the last two merges of 09-11: `51ec9a9` (employer sign-in moved to the header) and `04a93aa` (hide "Report this job" once reported). The API restarted around 09-11 too, and still reports `environment: development` — deliberate, see `deploy-checklist.md`. **The 12 register rows marked "awaiting prod retest" can now be retested on production.**

**Gates on `main` today:** `type-check` 0 · `lint` 0 (warnings only) · `verify-locales` structurally sound.

🔴 **The redesign brought 14 new bugs** → §4 rows 29–42. None blocks launch. The three that cost a user something:
- **A phone-only user cannot reset a password in production** (bug 30). The code is never sent by SMS, and the page rightly hides the echoed code in a production build — so the user waits for an SMS that never comes.
- **An Individual employer who types a GST number loses job posting** until an admin re-approves them (bug 32). The redesigned profile shows GST/CIN to Individual employers too, and the backend demotes on any GST change.
- **"Activate" on an expired job spends a credit with no warning** (bug 38). It is now the only button on the Expired tab.

**Also regressed:** TD-15 (the job feed's first screen) — the heading and subtitle TD-15 deleted are back (`job-feed/page.tsx:343-344`). And **the whole ledger page is English in the nine other languages**: its 23 strings were pasted in English, which `verify-locales` only warns about.

**Process:** 8 of the 40 commits carry a `Co-Authored-By` trailer, which `.claude/CLAUDE.md` forbids — `ac9ba0f` `b737e57` `6262372` `e208ee6` `1c227bb` `d8e77ad` `5c9c947` `9403ec0`.

**The other three surfaces — from their 2026-09-15 sync reports** (`C:\dev\Azkashine\Prosiddhi\_status-sync\backend.md`, `admin.md` and `mobile.md`). Each session read its own diffs and checked every API call against the backend.

- **Admin — 12 commits (prabhazkashine, 08-25 → 09-11).** The **invoice-PDF download is wired** (`af068a8`), so admin's "one task" is done. The console also grew a **SALES role**, a **Plans** page, **account restore**, a notification bell, and **Approve / Reject** for posts — now 19 page files and 93 client functions. 🔴 **Security, not tested on prod:** `POST /auth/forgot-password` with an email echoes the code whenever `EXPOSE_OTP_IN_RESPONSE` is on, which prod needs, and the email path has **no staff check**. Example: someone types a super-admin's email on `/admin/forgot-password`, reads the code off the screen, and sets a new password. 🟠 The **content scan has no UI any more** (`5b91f76` replaced it with Approve / Reject) — a scope call for Nazir. Three flows are broken: a wrong password shows "Session expired", staff phone reset can never work, and Manage Sales shows Restore to an ADMIN, who then gets a 403.
- **Mobile — 21 commits (sailaja, Krishna Kumar, 08-21 → 09-10). Still ~85%.** New: seeker documents, work-experience edit, change password, number of positions, the employer's own job view, applicant tabs, photos. **It has now run on Android** — `b68ca84` fixes a crash only a real device shows — but there is no recorded smoke test and no iOS run. 🔴 **The backend changed four contracts under it:** forgot password is broken (mobile sends `email`, the backend now wants `identifier`, so every request is a 400); the Shortlisted tab and the bookmark icon read retired fields; and My Jobs knows nothing of pre-moderation or expiry. `flutter analyze` is clean.

- **Backend — 71 commits (Asrar, 08-21 → 09-11, HEAD `c413dc0`).** Our three 08-21 commits (`61258ef`, `9dbb470`, `b5f6a0d`) are all still in. The big changes:
  - **Pre-moderation.** A new job is hidden until an admin approves it; the 30-day window starts at approval; a material edit to a live job sends it back to review. Example: raising the salary on a live post takes it offline until it is approved again.
  - **Re-listing an expired job costs 1 POST credit.**
  - **A bookmark is now a flag** (`isBookmarked`), not a status. Job status is now only `ACTIVE | INACTIVE | CANCELLED`, and application status lost `REVIEWED` and `BOOKMARKED`.
  - **Public job reads return ACTIVE jobs only** — by id, the feed, related jobs, recruiter contact.
  - The password rule is 8–64 with a special character; forgot/reset take `identifier`; **GST numbers must be valid and unique** (a clash is a 409).
  - A SUPER_ADMIN can edit and retire plans, with a buyer's terms frozen at purchase; a SALES role; a credit statement route.
  - **Prod's API already serves the new routes** — probed 2026-09-15: `/api/employers/me/credits/history` → 401, a made-up route → 404.
- 🔴 **Deploy trap.** The release is **19 hand-run SQL folders in a strict order**. **Never run `prisma db push` or `npm run prisma:migrate`** on a real database — both silently drop 5 unique indexes (one owner per org, one employer per GSTIN, …). **Nobody has recorded which of the 19 have run on production** — ask Asrar. The order is in `_status-sync\backend.md` §5, and (uncommitted) in the backend's `API_DOCUMENTATION.md` → Setup Instructions.
- 🔴 **Account takeover — §4 bug 3, reopened.** Phone OTPs have **no SMS code at all**, so prod runs with `EXPOSE_OTP_IN_RESPONSE` on. With it on, anyone who knows a seeker's or employer's phone number — or any account's email, staff included — can reset that password.
- **The portal is out of step with the backend in 5 places** → §4 rows 43–47. Worst: the employer's **Bookmarked tab is always empty**, and a **new job shows no "awaiting review" message** — the employer sees an Activate button that returns a 400.
- **Two items this file listed as open were long done:** the trial-lot invite fix was merged **2026-07-12** (`b28b827`), and CORS was locked down **2026-08-20** (`624fd30`).

---

**Previously (2026-08-18): all 10 languages ship on both clients — and auditing the English to translate it exposed 120 source-copy defects, several of them legal.**
...
## 0. At a glance — done vs left, per surface

*The short answer. Detail is in §2 (done) and §3 (left). Mobile keeps its own tracker at [`prosiddhi-mobile-app/docs/STATUS.md`](../../prosiddhi-mobile-app/docs/STATUS.md); everything else is answered here.*

| Surface | Done | Left |
|---|---|---|
| **Backend** | Feature-complete. HEAD `c413dc0` (2026-09-11). Auth (rebuilt 2026-08-03; phone password reset 09-10), jobs + taxonomy + **pre-moderation**, applications (a bookmark is a flag now), chat, search, **all of billing** (an editable plan catalogue, terms frozen at purchase, a credit statement), candidate DB, team seats, admin API + SUPER_ADMIN + **SALES** + audit log + restore, notification adapters, outbound email. *(From `_status-sync/backend.md`, 2026-09-15.)* | **1.** 🔴 **Phone OTP delivery is not built** — no SMS code exists, so prod needs `EXPOSE_OTP_IN_RESPONSE` on, which is an account-takeover path (§4 bug 3). A build task, not only config. **2.** 🔴 **Deploy: 19 hand-run SQL folders in order — never `prisma db push`.** Which ran on prod is unknown; ask Asrar. **3.** The plan catalogue lets a SUPER_ADMIN save a plan that breaks after payment (`durationDays` is not tied to the plan group). **4.** A failed phone reset can strand the number until someone removes a row by hand. **5.** Decide the OTP-enumeration question (§4 bug 22). **6.** Small: a `code` on registration errors (bug 18), one message for three causes (bug 19). **7.** No test suite in the repo. *(Done, though this row listed them until 2026-09-15: the trial-lot invite fix, `b28b827`, and CORS, `624fd30`.)* |
| **Portal** (web) | Feature-complete, and **redesigned to the Figma file 2026-08-25 → 09-11** (40 commits, Bharath Kumar). Every seeker + employer flow, chat, monetization, candidate DB, team seats, 10 languages, branding, the auth rework, the QA-defect pass — plus a seeker `/home`, a credit ledger, and email/phone change on the profile. | **19 closed · 6 open · 10 awaiting retest** → [`qa/defect-log.csv`](qa/defect-log.csv) *(re-counted 2026-09-15)*. **Production now runs the 09-11 build**, so the prod retest (TD-01) is unblocked. **19 new bugs** → §4 rows 29–47 (14 from the redesign, 5 from backend contract changes); the worst are 43 (Bookmarked tab always empty), 44 (no "awaiting review"), 30, 32 and 38. The location subsystem survived the redesign (TD-02/03/04/05/06/40/42). Still open: JWT → httpOnly cookie (hardening). ⭐ **The live work list is [teardown-fix-list.md](teardown-fix-list.md); it has a status index at the top.** |
| **Admin console** | Feature-complete. **19 page files · 93 client functions** *(was 10 · 55)*. QA-defect pass, SUPER_ADMIN screens, admin-adds-user, the audit log — and, new since 08-18, the **invoice-PDF download** (`af068a8`), a **SALES role** + Manage Sales, a **Plans** page, **account restore**, a notification bell, and **Approve / Reject** for posts. *(From `_status-sync/admin.md`, 2026-09-15.)* | **1.** 🔴 **Admin takeover through forgot-password** — the email path echoes the code and has no staff check (backend + config, Asrar + Nazir; not tested on prod). **2.** 🟠 The **content scan lost its UI** (`5b91f76`) — restore it or change the scope rule (Nazir). **3.** Three broken flows: a wrong password shows "Session expired"; staff phone reset never works; Restore is shown to an ADMIN, who gets a 403. **4.** Two client functions call removed `payment-status` routes (dead code today). **5.** Minor: the console accepts a narrower set of special characters than the backend. Nothing was click-tested. |
| **Mobile** (Flutter) | ~85% *(unchanged, re-checked 2026-09-15)*. Free product, candidate DB, team seats, chat, 10 languages, branding, registration rework — plus, since 08-21, seeker documents, work-experience edit, change password, number of positions, the employer's own job view and applicant tabs. **It has now run on Android** (`b68ca84`). | **1.** 🔴 **Backend drift** — forgot password is broken (sends `email`, the backend wants `identifier` → 400); the Shortlisted tab and bookmark icon read retired fields; My Jobs ignores pre-moderation and expiry; the password check lacks the special character. **2.** A **recorded** Android smoke test, and any iOS run. **3.** Invoices — never built. **4.** Google sign-in — switched off, needs Cloud-console clients. **5.** FCM push config. *Checkout is not a gap: D2 was resolved 2026-08-20, purchasing is web-only.* → [`prosiddhi-mobile-app/docs/STATUS.md`](../../prosiddhi-mobile-app/docs/STATUS.md) |
...
| Surface | Repo | State |
|---|---|---|
| **Backend** | `prosiddhi-backend` | ✅ **Feature-complete.** Everything the apps need is live, incl. the full billing system, the **SUPER_ADMIN role + admin-user CRUD + admin-adds-user + append-only audit log**, and **outbound email delivery** (OTP / invites / interview `.ics`). **Auth rebuilt 2026-08-03** (`2165880`…`09a88fc`, Asrar): password at registration, both contacts verified before the account exists, seeker email optional, `/set-password` deleted. Then 10 commits to `63632c2`: a **breaking** `Job` field removal (`fe246f1` — see §3 item 3d), job-expiry corrections, a **Prisma error-leak fix** (`890500a`), a rate-limit on the recruiter-contact reveal, and 404-not-500 on an unknown job id. **Then 71 more, 2026-08-21 → 09-11 (HEAD `c413dc0`, Asrar):** pre-moderation, paid re-listing, bookmark-as-flag, the job and application status clean-up, ACTIVE-only public reads, the 8–64 + special-character password rule, `identifier` on forgot/reset, unique GSTINs, an editable plan catalogue, the SALES role, restore, and a credit statement. ⚠️ **The release needs 19 hand-run SQL folders — never `prisma db push`.** See *Latest* and `_status-sync/backend.md`. |
| **Portal** (seeker + employer) | `prosiddhi-frontend` | ✅ **Feature-complete.** QA-defect pass done 2026-07-12 (2 criticals + 10 majors, audio removed, i18n cache bug, two open redirects); team invites + the public `/invite/<token>` page rebuilt the same day. **Registration + login rebuilt 2026-08-06** for the new auth contract — see §3 item 3a. **Branding fixed 2026-08-13** (`c2ed0a1`) — DEF-010 + DEF-011 closed. The 04-Aug QA run turned out to hold 32 issues, not 13 — items 14–32 were untriaged until 2026-08-13. **Redesigned to the Figma file 2026-08-25 → 09-11** (40 commits, Bharath Kumar) — see *Latest* at the top. **Re-counted 2026-09-15: 19 closed / 6 open / 10 awaiting retest** (→ [qa/defect-log.csv](qa/defect-log.csv)). ⚠️ **12 closures are still LOCAL-VERIFIED ONLY** — walked on a local stack by `scripts/smoke/retest-register.js`. **Production now runs the 09-11 build**, so the same walk can run on prod. ⚠️ Fix the script's DEF-023 check first: it looks for `/job-details/` links that My Jobs no longer has (§4 bug 42). The Contact Recruiter button and the dead job-form fields are reconciled (`5d982ac`). *(The cold-start invite fix is merged — `b28b827`, 2026-07-12. This row said it was blocked until 2026-09-15.)* |
| **Admin console** | `prosiddhi-admin` | ✅ **Feature-complete + QA-defect pass DONE** (2026-07-12). The two missing screens (**taxonomy**, **monetization**) are built, the reports queue and content scan are wired, the Revenue-card lie is fixed, and all 5 majors + the minors are done. **10 pages · 55 API functions** *(as of 2026-08-18)*. Every fix verified against a live backend. **Super-admin management, admin-adds-user, and the audit-log feed added (2026-07-27); the per-entity history panel is now built too (`b84f7d8`, `AuditTrail.tsx`) — this row previously said it was outstanding, which was stale.** *(The admin's own QA audit doc was fully resolved and deleted on 2026-08-18; its remaining state is this row and §0.)* **HEAD `161d94f` (2026-09-11) — 12 commits since 2026-08-18, by prabhazkashine.** The invoice-PDF download is wired (`af068a8`). New: a SALES role, Plans, account restore, a notification bell, and post Approve / Reject — now **19 page files · 93 client functions**. ⚠️ **No longer "nothing open":** one security flag (admin takeover through forgot-password), the content scan lost its UI, and three broken flows. See §0 and `_status-sync/admin.md`. |
| **Mobile app** | `prosiddhi-mobile-app` | 🟡 **~85% built** (Flutter). Free product, candidate DB, team seats, chat and **all 10 languages** all done. Registration reworked + **verified** 2026-08-06 (3 real defects found and fixed). **Branding shipped 2026-08-13** (`78b625a`) — it had Flutter's **default launcher icon** until then, plus the wrong app name under it. Six UI/UX fixes since (notification unread count + body truncation, message-tab and text-overflow fixes, redundant filters removed). **Checkout is not being built** (D2 resolved 2026-08-20: web-only). **Missing: invoices** (never built). **21 commits since 08-21** (sailaja, Krishna Kumar). **It has now run on Android** — `b68ca84` fixes a crash only a real device shows — but there is still no recorded smoke test and no iOS run. 🔴 **Four backend contract changes broke mobile flows**; forgot password returns a 400 on every request. → **`prosiddhi-mobile-app/docs/STATUS.md`** *(updated 2026-09-15)* |
...
1. **The backend runs in `development` mode on the public internet** — `/health` reports it. That means **OTPs are echoed in API responses** and raw error detail is exposed. It cannot just be flipped: in production the BE stops echoing OTPs, and with **MSG91 unconfigured nothing delivers them**, so registration becomes impossible for everyone. `NODE_ENV=production` and working OTP delivery must land **together**. *(2026-09-15: the echo is now controlled by `EXPOSE_OTP_IN_RESPONSE`, not `NODE_ENV` — TD-30. And there is **no SMS code for phone OTPs at all**, so "working OTP delivery" is a build task, not only config — §4 bug 3.)*
2. ~~CORS is still wide open~~ ✅ **Fixed 2026-08-20** (`624fd30`, TD-31) — an allowlist in the backend's `src/index.ts:80-95`, set by `CORS_ALLOWED_ORIGINS`.
...
2. **Mobile** — ~~the checkout~~ **(D2 resolved 2026-08-20: web-only purchasing, TD-36 removes the shopfront)**, invoices, and **getting it onto a device at all**.
3. ~~One open BE bug — the trial-lot fix~~ ✅ **merged 2026-07-12** (`b28b827`). **New in its place:** phone OTPs have no SMS delivery code, which keeps `EXPOSE_OTP_IN_RESPONSE` on in prod — and with it an account-takeover path (§4 bug 3).
...
### Backend
- **Auth** — phone-OTP, email+password, **Google OAuth**; email verification; forgot/reset password **by email or phone** (phone: no SMS is sent — §4 bug 3); the 8–64 + special-character password rule; soft-delete.
- **Jobs** — CRUD, **3-level taxonomy** validation, **pre-moderation** (a new post is hidden until an admin approves it; the 30-day window starts at approval; a material edit sends it back to review), re-listing an expired post costs 1 POST credit, ACTIVE-only public reads, recommendations, saved jobs, reports (once per seeker).
- **Applications** — apply (text only — audio removed 2026-07-12), status workflow (`PENDING | SHORTLISTED | REJECTED | ACCEPTED | WITHDRAWN`; a bookmark is a separate flag, `isBookmarked`), interviews.
- **Chat** — polling, text only, read receipts.
...
- **Monetization — all of it.** A plan catalogue a SUPER_ADMIN can edit and retire (a buyer's seats and terms are frozen at purchase), per-lot credit ledger + a credit statement route (`GET /employers/me/credits/history`), Razorpay checkout + webhook + client-verify (cannot double-grant), **GST invoices + PDF**, post-credit gate, delete-refund, credit wallet, 14-day trial grant, daily crons (30-day job window + 3-day post-expiry grace).
- **Candidate database** — FTS search, snippet gating (contact hidden), **atomic paid unlock** with dedupe, unlocked history.
- **Team seats** — roster, invite, accept, remove, revoke, public invite-peek.
- **Admin API** — queues, document verification, moderation (approve / reject), skills CRUD, **real revenue** from `PaymentHistory`, a **SALES** role (reads + Add User only), plan CRUD, account restore. The `payment-status` routes are gone.
- Rate limiting, webhook audit log.

### Portal (web)
- **Auth** — register (seeker + employer), one login form (phone or email + password, a code by SMS, Google), email verify, forgot/reset **by email or phone** (⚠️ phone reset is a dead end in prod — §4 bug 30), a live password checklist, role routing.
- **Seeker** — **home page** (`/home`: search + All / Recommended / Near By), job feed (search + a filter sidebar: city, job type, salary, department, role, urgent), job details, saved jobs, apply, my applications, contact-recruiter gate, report a job (hidden once reported), my interviews, profile (view/edit, **change email or phone with a code**).
- **Employer** — dashboard (job + application overview), post/manage jobs (taxonomy triple), their own job at `/employer/jobs/<id>`, candidate management, chat at `/employer/messages`, settings at `/employer/settings`, profile (view/edit, change email or phone).
- **Monetization** — pricing page, Razorpay checkout, credit wallet + expiry nudge, post-credit gate + upsell, top-up modal, **invoice history + PDF**, **credit ledger** (`/employer/ledger`, owner only).
...
- **Account deletion (PR-02)** — a **Delete account** card in Settings, on both `/settings` and `/employer/settings`. The user proves it is them by password or by Google, whichever the account has (the profile now says which: `hasPassword` / `hasGoogleLogin`), then the portal calls `DELETE /api/me`. An organisation owner with live teammates gets a second, explicit confirmation that shows the teammate count (409 `ORG_DELETE_CONFIRM_REQUIRED`). The backend does the real work (a soft delete; an admin can restore); the portal only asks. A wrong password or Google account keeps the session (`preserveSessionOnCode` in `api.ts`); any other 401 still logs out. See §4 row 48.
...
### Admin console
Login/guard, dashboard (**real revenue + 12-month trend + pending-verifications**), job-seeker management, employer management, document verification, post moderation (**Approve / Reject** since `5b91f76`; ⚠️ the content scan has no UI any more), skills catalog CRUD, **taxonomy management** (Category → Sector → JobTitle CRUD, M:N links, soft-delete tree), **monetization** (one payments table + a detail pop-up with the **invoice PDF**), a **reports queue** with resolve, and — since 08-25 — a **SALES** role + Manage Sales, a **Plans** page (SUPER_ADMIN), **account restore**, a notification bell, and forgot/reset password.

**19 page files · 93 client functions** *(2026-09-15; was 10 · 55)*. No mock data. ⚠️ Two dead client functions still call removed `payment-status` routes. Every write confirms itself; the header shows the real signed-in admin; no dead chrome. *(The whole QA-audit backlog was fixed 2026-07-12 — commits `ea1325d`, `e017f9b`, `2dc9cc9`, `ba93679`, `9dc2e0d`, `1ef4cf5`. The audit doc that tracked it was resolved and deleted 2026-08-18.)*
...
- ✅ The cold path's BE fix is on `main` (`b28b827`) — see item 1a.
→ Full detail in [MONETIZATION.md](MONETIZATION.md) §6.1–6.3.

**1a. ✅ BE — a trial credit lot blocked every new invitee — MERGED 2026-07-12** (`b28b827`; `team.service.ts:604-609` counts only SUBSCRIPTION + PACK lots). *(This item said "not merged" until 2026-09-15. The text below is history.)*
...
**3d. Portal — reconcile with the dropped `Job` fields** *(FE)* — ✅ **DONE 2026-08-18** (`5d982ac`).
*(This item read "🔴 NEW, OPEN, a live regression" until 2026-08-18. It is fixed — verified in the code: no `showEmailToSeekers` / `showPhoneToSeekers` reference survives outside two explanatory comments.)*

Backend `fe246f1` (06-Aug, Asrar) removed four `Job` columns: `duration`, `expiresAt`, `showEmailToSeekers`, `showPhoneToSeekers`. The portal was not updated for four weeks. `createJobSchema` is not `.strict()`, so Zod silently **stripped** the extra keys — nothing 400'd, which is exactly why it went unnoticed.

| Where | Was | Now |
|---|---|---|
| [job-details/[id]/page.tsx](../src/app/job-details/%5Bid%5D/page.tsx) | 🔴 Contact Recruiter gated on two fields the BE had dropped → both `undefined` → **the button never rendered on any job.** | Renders unconditionally. Employer contact is always shown to seekers by product decision; the reveal endpoint never filtered it. |
| [components/job/JobForm.tsx](../src/components/job/JobForm.tsx) | 🟠 Still collected **duration**, an **"Expires On"** date and the **two contact toggles**; the BE discarded all four. | All four controls removed (`5d982ac`, −34 lines). |
| [employer/jobs/[id]/edit/page.tsx](../src/app/employer/jobs/%5Bid%5D/edit/page.tsx) · [lib/api.ts](../src/lib/api.ts) | Hydrated / declared the four dead fields. | Gone; a comment at each site records why. |

**The fix was a deletion, not a feature.** *The expiry picker was always a lie — a job's life is `liveUntil`, 30 days per POST credit, whatever date the employer typed.*
...
- ✅ **The admin invoice-PDF gap is CLOSED (verified 2026-08-06).** `GET /api/admin/monetization/invoices/:id/pdf` exists at `admin.routes.ts:489`, shipped in `c8242ed`. ✅ **The console side is done too** (`af068a8`, 2026-08-27): **Download PDF** in the payment detail pop-up. ⚠️ It only finds the invoice among the employer's first 50.
...
- ⚠️ **2026-09-03: the scan UI was removed** (`5b91f76` — Approve / Reject replaced Scan / Violation / No-violation, and `adminAPI.posts.scan` has no caller). The text below is history. **Nazir's call:** restore it, or change the scope rule.
...
**3e. ✅ The location subsystem — both halves FIXED** *(FE + mobile; the only open piece is running TD-40's backfill on prod — see `teardown-fix-list.md`)*
...
> | **TD-03** the post-job form writes the job's coordinates | ✅ `6fd606c` `da49122` |
> | **TD-05** re-check the 20-point score | ✅ BE `b5f6a0d` (2026-08-21) |
> | **TD-38/04** mobile location | ✅ `4f37352` `b0ea4ab` `724b120` — never run on a device |
>
...
**11. Mobile — feature completion.** **~85% built** (this section previously said ~60%, which was stale by two sessions). The free product, **candidate database**, **team seats**, chat and **all 10 languages** are all done, the post-credit gate is in, registration is reworked and verified, and branding shipped 2026-08-13. **Remaining:** ~~the checkout~~ *(D2 resolved 2026-08-20: web-only)*, **the backend drift** (forgot password, Shortlisted, bookmark, My Jobs — `_status-sync/mobile.md` BE-1…BE-7), **invoices** (never built), **Google sign-in enablement** (built but switched off; needs Cloud-console OAuth clients), FAQ/Help, and **push** (FCM config). **It has now run on Android** (2026-09); a recorded smoke test and an iOS run are still owed. → **`prosiddhi-mobile-app/docs/STATUS.md`** is the live tracker and is **current to 2026-09-15** — it now carries the registration rework, the branding, the 10 languages and the six UI fixes. *(This line used to warn it was stale at 2026-07-13; that is no longer true.)*
...
**Migration:** run `prisma/migrations/2026-07-12_org_seats/up.sql` (data-preserving re-key + backfill; idempotent; reversible via `down.sql`) **before** `prisma db push`. Needs the `pgcrypto` extension (the migration creates it). ⚠️ **2026-09-15: never run `prisma db push` on a real database any more** — it silently drops 5 SQL-only unique indexes. The release now needs 19 hand-run SQL folders in a strict order → `_status-sync/backend.md` §5.

**Still needs EXTERNAL config before these light up in prod:**
- **MSG91** — `MSG91_AUTH_KEY` + **DLT-approved SMS template/sender IDs** + **Meta-approved WhatsApp template** + email template/domain. Until set, SMS/WhatsApp/Email deliveries record `SKIPPED`. ⚠️ This is true for *notifications* only: **phone OTPs have no SMS path at all**, even with MSG91 set (§4 bug 3).
...
**Left for the backend:** nothing from this session — the admin invoice-PDF route it asked for has since shipped (`c8242ed`), and the console wired its download button on 2026-08-27 (`af068a8`).
...
| 3 | BE 🔒 🔴 | **REOPENED 2026-09-15 — do not close. Account takeover while `EXPOSE_OTP_IN_RESPONSE=true`.** The echo is now controlled by that flag, not `NODE_ENV` (TD-30), and prod runs with it on because **no SMS code exists for phone OTPs** (`otp.service.ts` makes no send call). With it on, `POST /auth/forgot-password` returns the reset code — **by phone** for any seeker or employer since `1e57654` (09-10), and **by email for any account, staff included**, because the email path has no staff check (`accountRecoveryBlocker` checks status only). Example: type an admin's email on `/admin/forgot-password`, read the code off the screen, set a new password. **The fix is a build task** — SMS delivery for phone OTPs — and then turning the flag off. *(Was "recommend closing — dev-only" on 2026-08-06: true then, not now.)* |
...
| 11 | BE 🔒 | ✅ **CLOSE — the route now exists (verified 2026-08-06).** `GET /api/admin/monetization/invoices/:id/pdf` is live at `admin.routes.ts:489`, shipped in `c8242ed`. ✅ **The console button is wired too** (`af068a8`, 2026-08-27) |
| 12 | BE | ✅ **FIXED — merged 2026-07-12** (`b28b827`). A TRIAL credit lot made every brand-new account look like it "already runs its own workspace", so the cold-start invite 409'd. This row said "not merged" until 2026-09-15 → §3 item 1a |
...
| 25 | Portal | ✅ **FIXED 2026-08-18** (`5d982ac`) — the Contact Recruiter button never rendered on any job. It gated on `job.showEmailToSeekers \|\| job.showPhoneToSeekers`, two columns the BE dropped in `fe246f1` (06-Aug), so both read `undefined`. Not caught because `createJobSchema` isn't `.strict()` — the portal's extra keys were silently stripped and nothing errored. Now renders unconditionally → §3 item 3d |
| 26 | Portal | ✅ **FIXED 2026-08-18** (`5d982ac`) — the job form collected **duration**, an **"Expires On"** date and the **two contact toggles**; all four were discarded by the BE. All removed. The expiry picker was always a lie — a job lives `liveUntil`, 30 days per POST credit → §3 item 3d |
...
| 28 | Portal | ✅ **FIXED** (TD-24) — job details now resets the applied/saved state when the job changes. The 2026-09-04 page split kept the fix (`JobDetailsView.tsx:118-121`) |
| 29 | Portal + BE | ✅ **FIXED 2026-09-18** (`ba32b16`, branch `fix/password-max-length-64` — not yet merged to `main`). **The password maximum differed.** The portal allowed **128** characters (`lib/validation/passwordPolicy.ts:23`; all 10 locales said "8–128"); the backend has allowed only **64** since `2cbbe64` (09-11, `validators/password.ts:28`), verified by reading that file directly. `PASSWORD_MAX_LENGTH` is now 64, and the "8–128" copy is "8–64" across all 30 locale files (10 languages × auth/common/employerRegister). `type-check`, `lint` and `verify-locales` all pass |
| 30 | Portal + BE | ✅ **FIXED 2026-09-23** (backend `52313a2`, portal `29e933b`, merged as `d4c38bd`). Phone password reset used to fail silently: the backend said "we sent a code", but no SMS was ever sent. The backend now refuses every phone number with `503 PHONE_PASSWORD_RESET_DISABLED` (BE-A06), and the portal shows a translated "contact support" message with the support email, in all 10 languages, on both the send and reset steps. Email reset is unchanged. **Still true:** phone reset stays unavailable until SMS delivery is built (blocked on DLT approval), and the support address (`biz-ops@azkashine.com`) is not yet confirmed as the public one (R1-PM-01). Checked live in Chrome against the local backend, send step only, in Hindi and English. |
| 31 | Portal | ✅ **FIXED 2026-09-24** (`22cfe1f`, merged as `7b74140`). After a successful email or phone change on the seeker or employer profile, the session user now takes the freshly refetched email and phone, so Settings, checkout pre-fill and the invite page show the new values without a re-login. The verification modals were not changed. **Not fixed (backend):** `accept-invite` still compares the email in the login token, so after an email change accepting a team invite can still fail until the next login. Not tested in a browser; checked with `type-check`, `lint` and code, security and simplify reviews only. |
| 32 | Portal 🟠 | **NEW — open, needs a product call.** The redesigned employer profile (`f3be0ed`) shows GST and CIN to **Individual** employers too. The backend demotes any ACTIVE employer to `PENDING_ADMIN_APPROVAL` when GST or CIN changes, whatever the employer type (`employer.controller.ts:216-221`, `:282-286`), and posting needs ACTIVE. Example: an Individual employer types a GST number and cannot post a job until an admin re-approves them. The page does warn first. Decide: hide the fields for Individuals, or accept it |
| 33 | Portal | ✅ **FIXED 2026-09-24** (`7383957`, merged as `6511f2e`). The seeker profile DOB now has the same 18+ validation used during registration, with a translated `profile:seeker.errorDobAge` message across all 10 locales. Under-18/future DOB is blocked before the PUT request; empty DOB behavior remains unchanged. Previously the backend rejected under-18 DOBs but the profile showed only a generic **"Validation failed"** message. Browser verification used a mocked profile GET because the local backend Prisma client is stale; real reload persistence and the real backend 400 were not tested. |
| 34 | Portal | **NEW — open.** The status chip on **My Applications** is always green, so a REJECTED application looks like good news (`ApplicationCard.tsx:103` — deliberate per its comment; same on application details). `statusMeta` already holds the right colour per status |
| 35 | Portal | **NEW — open, low.** Job cards always say "**/ Month**" (`JobFeedCard.tsx:115`). So an hourly job reads monthly on Home, Job Feed and Saved Jobs, but "Hourly" on its detail page. Cards also use `createdAt` where the detail page uses `postedAt ?? createdAt`, so a re-listed job shows two different "posted" times |
| 36 | Portal + BE | **NEW — open.** **The ledger's "To" date drops most of the chosen day.** The page sends `2026-09-15`; the backend reads it as midnight UTC and keeps rows up to that moment (`credit.validator.ts:27`, `credit.service.ts:657`). So "To 15 Sep" stops at 05:30 IST, and From = To shows almost nothing. Fix on either side: send the end of the day, or treat `to` as the next day, exclusive |
| 37 | Portal | **NEW — open.** Ledger rough edges. (a) The **Ledger** menu item shows for every employer (`UserDropdown.tsx:240`), so a MEMBER seat clicks into a lock screen — the seat role is already on `/me/credits`. (b) **All 23 ledger strings are English in the nine other languages**, and the Reason and Description columns show raw English from the server. (c) It shares a 20-per-minute rate limit with candidate unlocks (`rate-limit.ts:50`), and every filter change sends a request |
| 38 | Portal 🟠 | **NEW — open.** On **My Jobs → Expired** the only button left is **Activate** (`c69b504`, `jobs/page.tsx:226-233`). Activate re-lists the job and **spends one job post credit** (`job.service.ts:1798-1845`) — with no confirm and no cost shown. It is the same "free reactivation" wording problem the copy audit found with "Renew". Also, an employer can no longer open an expired job or its applicants from the list. With no credit left, the backend returns a 402 (`INSUFFICIENT_POST_CREDITS`), shown as a raw toast with no upsell (`jobs/page.tsx:90`) |
| 39 | Portal | **NEW — open, low.** Header gaps after `6262372`: **Team** has no entry point on a phone (the link hides under 768 px, `EmployerHeader.tsx:77`), and **Plans** and **Invoices** are reachable only from the credit wallet, not the header. On a desktop every page still has an entry point |
| 40 | Portal | **NEW — open, low.** On the job feed, **department and role are now two separate pickers**, not a chain. A seeker can pick a role from another department and get zero results. TD-22 untangled the old cascade; the redesign removed it |
| 41 | Portal | **NEW — open, low.** On `/employer/jobs/<id>`, the error screen links to `/job-feed`, which is seeker-only (`JobDetailsView.tsx:242`) |
| 42 | Scripts | **NEW — open.** `scripts/smoke/retest-register.js:168` finds My Jobs' "View" link by `a[href^="/job-details/"]`. My Jobs now links to `/employer/jobs/<id>`, so the DEF-023 check will always say "no job rows" and hand it to a human. Fix the selector before the prod retest |
| 43 | Portal 🔴 | **NEW 2026-09-15 (backend drift) — open.** **The employer's Bookmarked tab is always empty.** It filters on `status=BOOKMARKED` (`employer/candidates/page.tsx:28`), a status the backend retired — a bookmark is now a flag, `isBookmarked`. The backend answers such a filter with an empty list and a 200, so nothing warns anyone. The candidate page has the same fault: the bookmark button shows only for PENDING / REVIEWED / BOOKMARKED (`candidates/[applicationId]/page.tsx:171`), and the bookmarked state reads `status` (`:349-350`). `isBookmarked` appears nowhere in the portal (`EmployerApplicationItem`, `api.ts:1495-1515`) |
| 44 | Portal 🟠 | **NEW (backend drift) — open.** **A new job shows no "awaiting review" message.** Since pre-moderation, every new job is `INACTIVE` + `PENDING_REVIEW` until an admin approves it. The employer is sent to My Jobs, sees the post under Active with an "Inactive" pill and an **Activate** button — and Activate returns a 400, "awaiting review" (`employer/jobs/new/page.tsx:41-42`, `employer/jobs/page.tsx:189-190`, `:228`) |
| 45 | Portal 🟠 | **NEW (backend drift) — open.** **Job details 404s for an applied or saved job that is no longer live.** `GET /api/jobs/:id` now returns only ACTIVE jobs to anyone but the owner and admins. So a seeker who opens an expired, deactivated or rejected job from My Applications or Saved Jobs gets an error page, not the job (`JobDetailsView.tsx:123`, `:139`; reached from `my-applications/[id]/page.tsx:275`, `saved-jobs/page.tsx:124-130`) |
| 46 | Portal | **NEW (backend drift) — open.** The GST number is checked for length only, and the registration number only for being non-empty (`employer/register/company-details/page.tsx:118`, `:122`; `employer/profile/page.tsx:299`). The backend now requires the GSTIN format and 5–50 characters, and returns a 409 `DUPLICATE_BUSINESS_IDENTIFIER` for a clash — which the portal shows as generic text. Example: `7576567FTYR6756` passes the form and fails the server |
| 47 | Portal | **NEW (backend drift) — open, low.** Dead status values. `EXPIRED_STATUSES = {FILLED, CLOSED, CANCELLED}` (`employer/jobs/page.tsx:35`, `:58`) and old names in `lib/applicationStatus.ts:25`, `:30`, `:55` — the backend can no longer send `FILLED`, `CLOSED`, `REVIEWED` or `BOOKMARKED`. The dashboard's **Reviewed** tile is now always 0, because the backend keeps `reviewed` at 0 |
| 48 | Portal | ✅ **FIXED — PR-02** (branch `feat/pr-02-delete-account`, not yet merged to `main`). **The Privacy Policy promised account deletion and the portal had no way to do it.** `legal.json` says a user can ask us to delete their account, and the backend already had `DELETE /api/me` (BE-A08, a Google Play requirement) — but nothing in the portal called it. Now built in Settings (see §2, Portal). Checked: `type-check`, `lint`, `verify-locales`, and 53 Playwright checks in real Chrome against the local backend (30 with the delete call mocked, 23 real, the last one ending in a real delete). ⚠️ **Deploy the backend first.** The Delete button needs the profile flags `hasPassword` / `hasGoogleLogin` (backend `e8d19d5`), and the delete route is backend `3521f34`. Until `e8d19d5` is live the flags are missing, so every account sees "contact support" instead of a button. ⚠️ **Not verified:** the Google button (Google will not render it on a test origin it has not allow-listed, so a real Google token was never sent); the 409 organisation-owner flow against a real owner with teammates (tested against a mocked reply only); and the new text in the nine non-English languages (machine-translated, not native-reviewed) |
| 49 | Portal | ✅ **DONE 2026-09-24 — C:PR-06** (`d8afbf4`, merged as `5f6efb6`). Seeker registration now encourages an email while keeping it optional: pressing Next with the email blank opens an "Add your email now?" prompt (**Add email now** / **Continue without email**), because an email is the only way to reset a forgotten password. Phone stays mandatory, and phone verification is skipped while the server says it is not required. **No profile-side email prompt is needed** — the manager's clarification puts the encouragement at registration only. **Still a deployment dependency:** the email code and the forgot-password reset email are sent by the backend's email channel, which still needs its MSG91 configuration (bug 17, §3 item 6). Checked in real Chrome against the local backend, 12 of 12 (the email-code send was mocked). The eight non-English strings are machine-translated, not native-reviewed |
...
- **Asrar** — backend
- **Sailaja** — mobile *(per `.claude/CLAUDE.md` Team and TD-32; this line said "unowned" until 2026-09-15)*
- **New names in the commit history since 2026-08-21 — roles not yet recorded, ask Nazir:** Bharath Kumar Srimanthula (all 40 portal commits, 2026-08-25 → 09-11) · prabhazkashine (admin) · Krishna Kumar (mobile)
- **Najeeb / Farhana** — QA
- **Nayan** — infra
...
| **3** | ~~**Admin: build + fix + docs**~~ ✅ **DONE 2026-07-12** | Taxonomy screen ✅ · monetization views ✅ · Revenue-card lie ✅ · the 5 majors ✅ · reports queue ✅ · content scan ✅. Six commits, each live-verified. The invoice-PDF route shipped, and the console button is wired (`af068a8`). | `prosiddhi-admin` |
...
**Still unowned / not scheduled:** outbound notifications (MSG91 SMS/WhatsApp/email + FCM push) — the BE adapters exist and no-op until keyed, so this is **external config**, not code. *(OpenAI content scan and the reports queue were on this list; both are built. The console still uses the reports queue; the scan lost its UI on 2026-09-03, `5b91f76`.)*
...
| **D2** | Mobile payments — in-app Razorpay or buy-on-web? | 🔓 **REOPENED AND RESOLVED 2026-08-20 by Nazir (PO): purchasing is WEB-ONLY.** Mobile does not sell anything; the shopfront is removed (TD-36). Reason: Play requires its own billing for our employer purchases, there is no B2B exemption, and the credits→"Job Posts" rename is not a defence — see [store-policy-assessment.md](store-policy-assessment.md). ~~Superseded: In-app Razorpay.~~ Mobile gets the full checkout (plans → `/api/billing/checkout` → `razorpay_flutter` → `/api/billing/verify-payment` → wallet). ⚠️ **Risk to verify:** Google Play / Apple may treat job-posting credits as *digital goods* and require their own in-app billing (15–30%). B2B services are often exempt — **confirm against store policy before building the checkout.** |
...
| **D7** *(2026-09-18)* | Release 1 registers seekers with an **unverified** phone. Phone is a unique key. So a number typed wrong in October sits on a live account, and in January — when verification turns on — its real owner cannot register with it. Who wins? | 🔒 **A verified claim always beats an unverified claim.** When a verified registration arrives for a number held by an unverified account, that account's phone is released and its user must supply a new one. Support can release an unverified number on request.<br><br>**Built now (BE-A09):** the manual lever only — `POST /api/admin/users/:userId/release-phone`, ADMIN or SUPER_ADMIN, audited (`PHONE_RELEASED`, recording who, when, which account and which number). It **refuses on a verified number** — taking a number its owner actually proved is a different, riskier act. `phoneVerified` now comes back on every admin user list and detail, so the console can show verification state and offer the action.<br><br>**NOT built:** the automatic January takeover. January builds that on top of this.<br><br>⚠️ **Admin console must warn before confirming.** Login arm 3 is phone + password, and a seeker who registered without an email — the normal shape of a Release 1 seeker, not an edge case — has that number as their **only** identifier. Releasing it does not remove one sign-in method, it leaves them nothing. The response carries `canStillSignIn`; when it is `false`, say so unmissably. The endpoint deliberately does **not** refuse in that case, because support still has to be able to resolve a genuine collision. Contract: `ADMIN_API_DOCUMENTATION.md`.
...
- **PJP-94** (OpenAI content scan) — BE shipped; the console used it (`e017f9b`) **until `5b91f76` removed the UI on 2026-09-03**.
- **PJP-102** (reports queue) — BE shipped **and** the admin console consumes it (`ba93679`).

The tickets that *are* genuinely still open map to §3 above: PJP-96/97/98 (notification channels — the BE adapters exist; this is external config), PJP-87 (staging/CI), the mobile stories, and the S3 hardening set. **New, no ticket yet:** the 19 new portal bugs (§4 rows 29–47), the reopened account-takeover bug (§4 bug 3), the admin flags in §0, and mobile's backend drift. *(The admin invoice-PDF download is done — `af068a8`.)* *(Taxonomy restore and PJP-99 admin audit log are both BUILT — close them.)*
