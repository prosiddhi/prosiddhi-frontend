...
| 31 | Portal | **NEW — open.** After a user changes their email on the profile, the session keeps the **old** email until the next login. Neither profile page refreshes the stored user (`profile/page.tsx:289`, `employer/profile/page.tsx:231`). Example: change `a@x.com` to `b@x.com` — Settings still shows `a@x.com` (`SettingsView.tsx:143`), checkout pre-fills it (`CheckoutModal.tsx:106`), and an invite for `b@x.com` is compared with the old address (`invite/[token]/page.tsx:89`) |
