...
| 29 | Portal + BE | **NEW 2026-09-15 — open, low.** **The password maximum differs.** The portal allows **128** characters (`lib/validation/passwordPolicy.ts:23`; all 10 locales say "8–128"). The backend allows **64** since `2cbbe64` (09-11, `validators/password.ts:28`). Example: a 70-character password turns every checklist item green, then the backend rejects it with a 400. The portal's comment saying it matches the backend is wrong |
