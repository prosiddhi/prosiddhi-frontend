...
| 30 | Portal + BE 🔴 | **NEW — open.** **Password reset by phone cannot work in production.** `2cbdb8e` lets `/forgot-password` take a phone number. But `otp.service.ts` never sends an SMS — in prod a phone code reaches the user only as the echoed `otp` in the API response. Register and verify screens show that echo; `/forgot-password` deliberately hides it in a production build (`forgot-password/page.tsx:37`). So the user reads "We sent a code to +91…" and nothing arrives. Email reset works. The real fix is to **build** SMS delivery for phone OTPs — no code for it exists, so this is not only config (bug 3) |
