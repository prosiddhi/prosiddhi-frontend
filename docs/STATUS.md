...
Other docs: [PRODUCT.md](PRODUCT.md) (what we're building) · [MONETIZATION.md](MONETIZATION.md) (pricing rules + billing) · [DEPLOY.md](DEPLOY.md) (deploy + go-live) · [go-live-config.md](go-live-config.md) (deploy dependencies) · [qa/manual-testing-walkthrough.md](qa/manual-testing-walkthrough.md) (manual test guide).
...
⚠️ **Web and mobile registration flows have diverged** — mobile (`37dedcb`) shipped a different order the same day. Web is canonical; **aligning mobile is unowned and not scheduled.**
...
| 20 | Mobile | **NEW — open, unowned.** Mobile (`37dedcb`) shipped a **different registration order** from the web on the same day. Web is canonical (decision A-1); aligning mobile is **not scheduled and has no owner** |
