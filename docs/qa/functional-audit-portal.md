...
| E22 | Team roster + invite (Pro) | `/employer/team` as the OWNER → invite by email → copy link; revoke a pending invite; remove an accepted member | Roster shows **two groups**: members (Owner pill / **Active** / **Suspended**) and **Pending invites**. One-shot link is copyable and points at `/invite/<token>`. Usage + free-seat count update. Full plan → upgrade prompt. **Revoke** kills a pending invite (frees the seat); **Remove** revokes an accepted member — they are different controls and must not be crossed |
| E22b | Team page as a MEMBER (not the owner) | Sign in as an accepted teammate → `/employer/team` | Roster is visible **read-only**: no invite form, no remove buttons, "only the owner can invite or remove" note. If the seat is SUSPENDED (owner's plan shrank), an amber "your seat is paused" banner appears |
| E23 | Accept an invite — WARM (invitee already has an employer account) | Open `/invite/<token>` signed in as the invited employer | Shows company + the bound email + expiry → **Accept** → joins → dashboard. Signed in as the **wrong** account or as a **job seeker** → clear message + "sign in with a different account", **not** a raw error. Expired/used/garbage token → "this invite can't be used" + a way out (never a dead end) |
| E23b | Accept an invite — COLD (invitee has NO account yet) | Invite a **fresh** email → open `/invite/<token>` signed out → "Create your account" → complete registration | The token survives registration and the invite **auto-accepts on return** — the link is clicked **once**, never relayed twice. Then confirm the **shared wallet**: the new member's `/employer` credit balance is the **owner's org balance**, not their own trial 1/3. ⚠️ **Requires the BE fix on `fix/invite-trial-lot-blocks-cold-path`** — without it this path 409s every time (STATUS §3 item 1a) |
| E23c | Invite link is single-use + email-bound | Reuse an accepted token; open a token while signed in as a different employer | Replay → "already used"; different account → email-mismatch message. Enforced by the BE (403/400), not just hidden in the UI |
