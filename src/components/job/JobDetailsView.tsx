...
interface JobDetailsViewProps {
  /** Text on the back button. Each role's page computes its own — a seeker's
   *  depends on where they came from (Home/Job Feed/Saved Jobs/an application),
   *  an employer's is always "Back to My Jobs". */
  backLabel: string
 ...
 *
 * The seeker-only actions stay gated on `isSeeker`, computed here from the
 * session rather than passed in — on the employer route that's guaranteed
...
export function JobDetailsView({ backLabel, onBack }: JobDetailsViewProps) {
  ...
  // Every seeker-only ACTION is hidden from an employer viewing their own post:
  // an employer must not apply to, save, or report a job — and the backend
      ...
      <main className="flex-1 pt-[clamp(16px,5.33px_+_1.67vw,32px)] pb-[clamp(24px,8px_+_2.5vw,48px)]">
        <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-[120px]">
          ...
          <div className="max-w-[1920px] mx-auto">
            <button onClick={onBack} className="flex items-center gap-2 text-black hover:text-primary-50 transition-colors mb-[clamp(16px,8px_+_1.25vw,28px)]">
            ...
            {!loading && !error && job && (
              ...
              {/* Action Buttons — seeker-only. An employer viewing their own post
                  gets the detail, not actions the backend would refuse.
